...
- `album`/`albumArtist`/`genre` prefer the same defensive taglib read the reconcile
  uses (`MetadataReader(uri).toExternalSongTags()`, falling back to the retriever
  probe), so the album id derived at import (`externalAlbumId(album, albumArtist)`)
  matches the one the first-play reconcile recomputes — otherwise played and
  unplayed songs of one album land under different ids and the album splits in the
  Albums tab.
